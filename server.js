...
app.get(`${fullAPIRoot}/todos/:id?`, (req, res) => {
  let query = {};
  const { id } = req.params;
  if (id) {
    query._id = id;
  }
  Todo.find(query).exec(function (err, todos) {
    if (err) return res.status(500).send(err);
    return res.status(200).send(todos);
  });
});

app.post(`${fullAPIRoot}/todos/`, (req, res) => {
  ...
  if(typeof todoData.title !== 'string') {
    return res.status(400).send("TITLE_MUST_BE_A_STRING");
  }

  if (!todoData.duration) {
    return res.status(400).send("NO_DURATION_PROVIDED");
  } 
  if(typeof todoData.duration !== 'string') {
    return res.status(400).send("DURATION_MUST_BE_A_STRING");
  }

  if(todoData.done && typeof todoData.done !== 'boolean') {
    return res.status(400).send("DONE_MUST_BE_A_BOOLEAN");
  }
...
app.put(`${fullAPIRoot}/todos/:id`, (req, res) => {
  const updateData = req.body;
  console.log(`Updating ${req.params.id}`, updateData);

  const isEmpty = updateData // 👈 null and undefined check
  ...
  Todo.updateOne({ _id: req.params.id }, updateData, function (err, result) {
    if (err) {
      return res.status(500).send(err);
    }
    console.log("result", result);
...
app.delete(`${fullAPIRoot}/todos/:id`, (req, res) => {
  console.log("todoToBeDeleted", req.params.id);
